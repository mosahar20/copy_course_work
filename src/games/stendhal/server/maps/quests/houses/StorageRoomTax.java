 ...
 */
class StorageRoomTax implements TurnListener {
	/** The base amount of tax per month. */
	protected static final int BASE_TAX = 100;

	private static final Logger logger = Logger.getLogger(StorageRoomTax.class);
	 ...
	 */
	protected int getTaxDebt(final StorageRoomPortal portal) {
	...
	protected int getUnpaidTaxPeriods(final Player player) {
		final StorageRoomPortal portal = StorageRoomUtilities.getPlayersRoom(player);
		int payments = 0;
	 ...
	 * @return number of periods
	 */
	private int getUnpaidTaxPeriods(final StorageRoomPortal portal) {
	...
	public void onTurnReached(final int turn) {
		...
		for (final StorageRoomPortal portal : StorageRoomUtilities.getStorageRoomPortals()) {
				...
				if (payments > MAX_UNPAID_TAXES) {
					...
					if (payments == MAX_UNPAID_TAXES) {
						...
						remainder = " This is your final warning, if you do not pay your taxes within a month then "
							+ "your storage room will be made available for others to buy, and the locks will be changed. "
							+ "You will be unable to access your storage room or its chest.";

					} else {
						remainder = " Pay promptly, as I charge interest on debts owed. And if you fail to pay for "
							+ Integer.toString(MAX_UNPAID_TAXES + 1) + " months, your storage room will be repossessed.";
					}
					notifyIfNeeded(owner, "You owe " +  Integer.toString(getTaxDebt(payments)) + " money in storage room tax for "
							+ Grammar.quantityplnoun(payments, "month", "one")
							+ ". You may come to the storage depot to pay your debt." + remainder);
				}
			}
		}
	}
	 ...
	 */
	private void confiscate(final StorageRoomPortal portal) {
		notifyIfNeeded(portal.getOwner(), "You have neglected to pay your storage room taxes for too long. "
					   + "Your storage room has been locked and you may collect it by paying a fee.");
	...
	private void setupTaxman() {
		...
		taxman.addReply("tax", "All storage room owners must #pay taxes to the state.");
		...
		taxman.add(ConversationStates.ATTENDING,
				   ...
				   "According to my records you don't currently owe any tax. Storage owners will get notified by "
