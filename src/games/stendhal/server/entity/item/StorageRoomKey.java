 ...
 */
public class StorageRoomKey extends Item {
	...
	public StorageRoomKey(final String name, final String clazz, final String subclass,
	 ...
	 */
	public StorageRoomKey(final StorageRoomKey key) {
		super(key);
	}
	...
	public String describe() {
		...
		if (info[2].length() > 0) {
			return "You see a key to " + Grammar.suffix_s(info[2]) + " storage room, " + info[0] + ".";
		} else {
	...
	public String getName() {
		final String[] info = getInfoString().split(";", -1);
		if (info[2].length() > 0) {
			return Grammar.suffix_s(info[2]) + " storage room key";
		} else {
			return  info[0] + " key";
		}
	}
	 ...
	 */
	public boolean matches(final String storageroomId, final int number) {
		final String[] info = getInfoString().split(";");

		int keyNumber = Integer.parseInt(info[1]);
		return (info[0].equals(storageroomId) && keyNumber == number);
	}
