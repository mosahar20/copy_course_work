 ...
 */
public class StorageRoomPortal extends AccessCheckingPortal {
	private static final String RPCLASS_NAME = "storage_portal";
	 ...
	 */
	public StorageRoomPortal(final String doorId) {
		super("The door is locked.");
		setRPClass(RPCLASS_NAME);
		put("type", "storage_portal");
		put(DOOR_ID, doorId);
		put(OWNER, "");
		put(LOCK_NUMBER, 0);
		store();
	 ...
	 */
	public StorageRoomPortal(final RPObject object) {
		super(object);
		setRPClass(RPCLASS_NAME);
		put("type", "storage_portal");
	...
	public String describe() {
		...
		if (owner.length() > 0) {
			description = "Storage room belonging to " + owner + ".";
		} else {
			description = "Empty Storage Room!";
		}
	...
	private boolean keyMatches(RPObject obj) {
		if (obj instanceof StorageRoomKey) {
			final StorageRoomKey key = (StorageRoomKey) obj;
